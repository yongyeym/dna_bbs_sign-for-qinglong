...
CONTENT_LENGTH_NUM = {"forum/list" : "78", "forum/like" : "146", "shareTask" : "10", "getPostDetail" : "58", "TaskProcess" : "10", "signIn" : "10", "signInGame" : "38", "signin/show" : "10", "haveSignInNew" : "10"}
...
class ProcessInterrupt(Exception):
    ...
    使用 raise ProcessInterrupt("title", "content") 主动抛出
    """
    def __init__(self, title, content = "请查看日志！"):
        self.title = title
        self.content = content
        super().__init__(self.title)
    def __str__(self):
        e2str = f"【{self.title}】二重螺旋·签到\n\n详情为：" + self.content  # 定义直接输出e时的显示文本
        return e2str

def get_dnabbs_taskprocess():
    """
    ...
    data = {
        ...
        'userId': "760623756170231810"  # 用户ID
    }
    ...
    if response["code"] == 200:
        ...
        raise ProcessInterrupt("Cookie失效", "Cookie失效，请更新环境变量dnabbs的值！")
    elif response["code"] == 500:
        raise ProcessInterrupt("失败", f"请求被拒绝，请重新尝试或检查日志！错误信息：{response['msg']}")
    else:
        raise ProcessInterrupt("失败",f"请求出现异常或被拒绝！Code {response['code']} - {response['msg']}")
...
def do_like(postId,toUserId):
    ...
    返回JSON：
    {"code":200,"data":{"success":true},"msg":"请求成功","success":true}
    """
...
def do_signin_game(periodId, dayAwardId, award):
    ...
    返回JSON：
    {"code":200,"data":{"signinTimeNow":"1","sendDayAward":true},"msg":"请求成功","success":true}
    """
    message = ""
...
def format_process(process_num):
    """
    对JSON数据中process百分比数据进行格式化处理
    原数据为转为字符串的浮点数，小数点后有约10位，将其处理成百分比显示的最多2位小数位的字符串返回
    """
    ...
    if ACCOUNT is None:
        ...
        try:
            ...
            while restart_flag and attempt < 3:
                ...
                read, like, share, bbs_sign, comment = get_dnabbs_taskprocess()
                time.sleep(2)
